...
import com.straccion.socialapp.android.coomon.util.Constants.DEFAULT_REQUEST_COMMENTS_PAGE_SIZE
        ...
        return DefaultPagingManager(
            onRequest = { page ->
                getPostCommentsUseCase(
                    postId = postId,
                    page = page,
                    pageSize = DEFAULT_REQUEST_COMMENTS_PAGE_SIZE
                )
            },
            onSuccess = { comments, _ ->
                commentsUiState = commentsUiState.copy(
                    comments = commentsUiState.comments + comments,
                    endReached = comments.size < DEFAULT_REQUEST_COMMENTS_PAGE_SIZE
                )
            },
            onError = { message, _ ->
                commentsUiState = commentsUiState.copy(
